...
class Player {
public:
	Player(bool is_facing_right = 1) : is_facing_right(is_facing_right) {
		current_animation = is_facing_right ? &animation_idle_right : &animation_idle_left;

		timer_attack_cd.set_wait_time(attack_cd);
		timer_attack_cd.set_one_shot(1);
		timer_attack_cd.set_callback([&]() {
			can_attack = 1;
			});

		timer_invulnerable.set_wait_time(500);//750
	...
	virtual void on_update(int delta) {
		...
		if (dir) {
			if(!is_attacking_ex) is_facing_right = dir > 0;
			current_animation = is_facing_right ? &animation_idle_right : &animation_idle_left;
			float distance = dir * run_velocity * delta;
			on_run(distance);
		}
		else {
			current_animation = is_facing_right ? &animation_idle_right : &animation_idle_left;
	...
	virtual void on_jump() {
		if (velocity.y || is_attacking_ex)return;
		...
		if (velocity.y > 0) {
			for (Platform& platform : platform_list) {
				...
				if (is_collide_x && is_collide_y) {
					...
					float last_tick_foot_post_y = position.y + size.y - delta_pos_y;
					if (last_tick_foot_post_y <= shape.y) {
...
protected:
	...
	IMAGE img_sketch;
	bool is_invulnerable = 0;			// 是否无敌
	bool is_showing_sketch_frame = 0;	// 当前帧是否显示剪影
